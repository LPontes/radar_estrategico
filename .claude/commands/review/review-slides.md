...
name: review-slides
description: |
  Review an sa slide deck for quality issues. Invokes sa-slide-reviewer for independent
  validation against 8 rule categories. Optionally auto-fixes with sa-slide-fixer.
---

# Review Slides Command

> Independent quality review for sa HTML slide decks
...
This command invokes the **sa-slide-reviewer** agent for independent, read-only validation of any slide deck against the sa quality KB. Optionally pairs with **sa-slide-fixer** for automated corrections.
...
### Step 2: Run sa-slide-reviewer

Spawn the **sa-slide-reviewer** agent with this prompt:
...
1. Spawn **sa-slide-fixer** agent with the review report and deck path
2. Fixer applies surgical corrections for all CRITICAL + ERROR findings
3. Fixer applies WARNING fixes with confidence >= 0.85
4. After fixing, spawn **sa-slide-reviewer** again for confirmation pass
...
IF all categories pass with 0 CRITICAL, 0 ERROR:
  1. Celebrate: "Deck meets sa production quality bar"
...
|-------|-----------------|
| **sa-slide-reviewer** | Independent read-only validator (always invoked) |
| **sa-slide-fixer** | Surgical corrections (only with --fix flag) |
...
- `sa-slide-reviewer` agent: `.claude/agents/domain/sa-slide-reviewer.md`
- `sa-slide-fixer` agent: `.claude/agents/domain/sa-slide-fixer.md`
