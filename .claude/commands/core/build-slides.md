...
name: build-slides
description: |
  Full pipeline for building sa slide decks: plan → build (chunked) → review → fix → assemble.
...
│  └─ Set output path                                              │
│                                                                   │
│  Step 1: PLAN (sa-slide-planner)                               │
│  ├─ Reads content + KB → structured slide-map                     │
│  └─ Defines: types, visuals, chunks, accent words                │
│                                                                   │
│  Step 2: BUILD + REVIEW LOOP (per chunk)                         │
│  ├─ sa-slide-builder generates 5-7 slides                      │
│  ├─ sa-slide-reviewer validates chunk                          │
│  ├─ sa-slide-fixer corrects if needed                          │
...
Spawn the **sa-slide-planner** agent:
...
- .claude/kb/sa-slides/slide-types.md
- .claude/kb/sa-slides/advanced-visuals.md
- .claude/kb/sa-slides/{palette_file}
...
- SVG slides → .claude/kb/sa-slides/advanced-visuals.md
- Card/stat slides → .claude/kb/sa-slides/component-library.md
- All chunks → .claude/kb/sa-slides/quality-rules.md
- All chunks → .claude/kb/sa-slides/{palette_file}
- Chunk 1 only → .claude/kb/sa-slides/template.md (for <head> and structure)
- Chunk 1 only → .claude/kb/sa-slides/slide-engine.md (for JS)
...
If review has CRITICAL or ERROR findings, spawn **sa-slide-fixer** agent:
...
Spawn **sa-slide-reviewer** one final time on the complete assembled file:
...
|-------|-------|-------|------|
| **sa-slide-planner** | Step 1 | Sonnet | Content architecture → slide-map |
| **sa-slide-builder** | Step 2a | Opus | HTML generation per chunk |
| **sa-slide-reviewer** | Step 2b, 4 | Sonnet | Independent validation (read-only) |
| **sa-slide-fixer** | Step 2c | Sonnet | Surgical corrections |
...
- Quality rules KB: `.claude/kb/sa-slides/quality-rules.md`
