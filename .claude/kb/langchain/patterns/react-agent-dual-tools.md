...
> **Purpose**: EDA LangChain agent with supabase + qdrant tools that autonomously routes SQL vs semantic queries
> **MCP Validated**: 2026-04-12

## When to Use

- Day 3 single-agent EDA with autonomous query routing
...
```python
"""EDA Day 3: ReAct agent with dual-store routing."""
...
def supabase_execute_sql(query: str) -> str:
    ...
    Args:
        query: SQL query string to execute against the EDA database.
    """
...
def qdrant_semantic_search(question: str) -> str:
    ...
    # result = mcp_qdrant.search(collection="EDA_reviews", query=question)
...
def ask(question: str) -> str:
    """Ask EDA a question — it routes to the right store."""
    result = agent.invoke({
...
- [Supabase Ledger Queries](../../supabase/patterns/EDA-ledger-queries.md)
