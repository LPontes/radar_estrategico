...
Tools are Python functions decorated with `@tool` that LangChain agents can invoke. The `docstring` is critical — the LLM reads it to decide WHEN to call each tool. For EDA, two tools cover the dual-store architecture: `supabase_execute_sql` for exact data (The Ledger) and `qdrant_semantic_search` for meaning-based search (The Memory).
