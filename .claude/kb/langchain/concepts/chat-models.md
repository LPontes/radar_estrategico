...
> **Purpose**: Configure Claude as the LLM backbone for EDA agents with tool-binding
> **Confidence**: 0.95
> **MCP Validated**: 2026-04-12

## Overview

`ChatAnthropic` is the LangChain wrapper for Claude models. It supports `bind_tools` to attach tool schemas that the LLM can invoke, and `streaming=True` for token-by-token output in Chainlit. For EDA, use `claude-sonnet-4-20250514` with `temperature=0` for deterministic tool routing.
