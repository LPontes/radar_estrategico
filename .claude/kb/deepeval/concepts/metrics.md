...
> **Purpose**: DeepEval metrics for evaluating EDA tool selection and answer quality
> **Confidence**: 0.95
> **MCP Validated**: 2026-04-12

## Overview

DeepEval provides four primary metrics for EDA evaluation. `ToolCorrectnessMetric` is deterministic (no LLM judge needed). The others (`AnswerRelevancyMetric`, `FaithfulnessMetric`, `GEval`) use an LLM-as-judge. All metrics expose `.score` (float 0-1) and `.reason` (string) after `.measure()`.
