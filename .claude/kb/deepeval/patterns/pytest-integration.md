...
> **Purpose**: Integrate EDA evaluations into pytest CI with parametrized test cases
> **MCP Validated**: 2026-04-12

## When to Use

- Automated testing in CI/CD pipeline
- Regression testing after changes to EDA routing logic
- Per-test-case pass/fail reporting with pytest output

## Implementation

```python
"""tests/test_EDA.py — EDA evaluation via pytest + DeepEval."""
import pytest
...
# ---------------------------------------------------------------------------
# Dataset — representative EDA routing scenarios
# ---------------------------------------------------------------------------
_RAW_CASES = [
    LLMTestCase(
...
def test_EDA_routing(test_case: LLMTestCase) -> None:
    """Assert EDA routes to the correct tool."""
...
def test_EDA_relevancy(test_case: LLMTestCase) -> None:
    """Assert EDA answers are relevant to the question."""
    assert_test(
        test_case,
...
| `AnswerRelevancyMetric.model` | `"claude-sonnet-4-20250514"` | Match EDA stack |

## Example Usage

```bash
# Run with deepeval CLI (recommended)
deepeval test run tests/test_EDA.py

# Run with pytest
pytest tests/test_EDA.py -v

# Run only routing tests
pytest tests/test_EDA.py -k "routing" -v

# Stop on first failure
pytest tests/test_EDA.py -x -v
```
