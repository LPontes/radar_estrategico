...
`Settings` is a global singleton that controls default behavior across all LlamaIndex operations. Set `embed_model` to choose the embedding provider, `llm` for response synthesis, and `chunk_size`/`chunk_overlap` for document splitting. For EDA, we use `FastEmbedEmbedding` (BAAI/bge-base-en-v1.5) which runs locally with 768 dimensions and requires no API key.
